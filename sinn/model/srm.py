...
class Activity(Model):
    ...
    def η2_fn(self, t):
        ...
        t = shim.add_axes(t, len(self.A.shape)*2, 'right')
    ...
    def __init__(self, params,
        ...
        self.JsᕽAᐩI = history.Series(self.A.t0,
                                    ...
                                    self.A.dt,
                                    shape = self.A.shape,
                                    f = lambda t: self.Js*A[t] + self.I[t])
        ...
        M = int(shim.round((self.memory_time + 1) / self.A.dt))
        ...
        abs_refrac_idx_len = shim.cast_varint16( shim.round(params.τabs / self.A.dt) ) - 1
            # -1 because tarr_η starts at dt
        ηsides = abs_refrac_idx_len.shape
        self.η = shim.ifelse((abs_refrac_idx_len > 0).any(),
                             lib.stack(
                                 [ lib.stack(
                                    [ shim.set_subtensor(η2[-aril_el:], shim.inf)
                                       for aril_el in aril_row ] )
                                    for aril_row in abs_refrac_idx_len] ),
                             η2)
    ...
    def compute_range_a(self, t_array):
        ...
        shim.check( t_array[0] == self.cur_tidx + 1 )
